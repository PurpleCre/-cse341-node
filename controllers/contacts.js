...
const updateContact = async(req, res) => {
    ...
    const response = await mongo.getDatabase().db().collection("contacts").replaceOne({ _id: contactId}, contact);
...
const deleteContact = async(req, res) => {
    //#swagger.tags=['Contacts']
    const contactId = new ObjectId(req.params.id);
    const response = await mongo.getDatabase().db().collection("contacts").deleteOne({ _id: contactId});
